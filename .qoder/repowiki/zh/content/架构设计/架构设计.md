...
## 更新摘要
**所做更改**   
- 增强了架构图和流程图，提供更清晰的系统结构关系展示
- 优化了数据流向图，帮助开发者理解组件间的数据传递模式
- 新增了详细的组件交互时序图，展示事件驱动通信的具体流程
- 完善了分层架构的可视化表示，突出各层职责边界
- 增加了扩展点设计的架构图，便于后续功能扩展
...
graph TB
subgraph "应用入口层"
Program["Program.cs<br/>应用程序启动"]
end
subgraph "表现层 (WinForms)"
MainForm["MainForm.cs<br/>主窗体协调器"]
F1["Form1.cs<br/>辅助窗体"]
F2["Form2.cs<br/>辅助窗体"]
PJF["PointJumpForm.cs<br/>点位跳转窗体"]
TVF["TrajectoryViewForm.cs<br/>轨迹视图窗体"]
end
subgraph "领域层 (业务逻辑)"
Hub["XyzControllerHub.cs<br/>通信中心"]
AxisCtrl["AxisController.cs<br/>轴控制器"]
JogSvc["AxisJogService.cs<br/>点动服务"]
JogMode["JogMode.cs<br/>点动策略"]
end
subgraph "基础设施层 (WPF宿主)"
HostLaunch["WpfHostLauncher.cs<br/>宿主启动器"]
WpfMW["MainWindow.xaml.cs<br/>WPF主窗口"]
WpfPage["WpfPage.cs<br/>WPF页面"]
end
subgraph "基础设施层 (自定义控件)"
AB["AxisBar.cs<br/>轴条控件"]
DL["DroLabel.cs<br/>数值显示"]
JB["JogButton.cs<br/>点动按钮"]
JP["JoystickPad.cs<br/>摇杆控件"]
XYV["XYView.cs<br/>二维视图"]
ZBV["ZBarView.cs<br/>Z轴视图"]
MH["MathHelper.cs<br/>数学工具"]
PH["PaintHelper.cs<br/>绘制工具"]
end
Program --> MainForm
MainForm --> Hub
Hub --> AxisCtrl
