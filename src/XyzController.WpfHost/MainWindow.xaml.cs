...
namespace XyzController.WpfHost
{
    ...
    public partial class MainWindow : Window
    {
        ...
        private void Window_Loaded(object sender, RoutedEventArgs e)
        {
            ...
            // ★ 固定窗口尺寸：仅启动时把小屏放不下的初始尺寸一次性钳制到工作区内；
            //   之后无论切换到哪个页面，窗口尺寸都保持不变（只切 Visibility，绝不 resize）。
        ...
        /// 所有页面统一 Dock=Fill 填充同一个固定尺寸的内容区：窗口尺寸恒定，
        /// 页面靠自身的 Dock/Anchor 弹性布局与 AutoScroll 适配窗口，而非窗口适配页面。
        /// </summary>
        private void EmbedFormIntoHost(Form form, WindowsFormsHost wfHost)
        {
            ...
            // 清除设计期 MinimumSize：页面必须能收缩到窗口内容区，不能反向撑大窗口。
            form.MinimumSize = new System.Drawing.Size(0, 0);
            // 内容超出可视区时由页面自身滚动条兜底（Form 默认为 true，此处显式保证）。
        ...
        /// <summary>
        /// 启动时一次性钳制：XAML 中的固定初始尺寸超出当前屏幕工作区时（小屏/低分辨率），
        /// 收缩到工作区内并重新居中。只在启动时执行一次；
        /// 之后切换页面窗口尺寸保持恒定，绝不 resize。
        /// </summary>
        private void ClampWindowToWorkAreaOnce()
        {
            ...
            if (this.Width > workArea.Width) { this.Width = workArea.Width; changed = true; }
            if (this.Height > workArea.Height) { this.Height = workArea.Height; changed = true; }
